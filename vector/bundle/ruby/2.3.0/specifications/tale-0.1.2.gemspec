# -*- encoding: utf-8 -*-
# stub: tale 0.1.2 ruby lib

Gem::Specification.new do |s|
  s.name = "tale"
  s.version = "0.1.2"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib"]
  s.authors = ["Chester How"]
  s.date = "2018-01-21"
  s.email = ["chesterhow@gmail.com"]
  s.homepage = "https://github.com/chesterhow/tale"
  s.licenses = ["MIT"]
  s.rubygems_version = "2.5.2.1"
  s.summary = "Tale is a minimal Jekyll theme curated for storytellers."

  s.installed_by_version = "2.5.2.1" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<jekyll>, ["~> 3.6"])
      s.add_runtime_dependency(%q<jekyll-paginate>, ["~> 1.1"])
      s.add_development_dependency(%q<bundler>, ["~> 1.12"])
      s.add_development_dependency(%q<rake>, ["~> 10.0"])
    else
      s.add_dependency(%q<jekyll>, ["~> 3.6"])
      s.add_dependency(%q<jekyll-paginate>, ["~> 1.1"])
      s.add_dependency(%q<bundler>, ["~> 1.12"])
      s.add_dependency(%q<rake>, ["~> 10.0"])
    end
  else
    s.add_dependency(%q<jekyll>, ["~> 3.6"])
    s.add_dependency(%q<jekyll-paginate>, ["~> 1.1"])
    s.add_dependency(%q<bundler>, ["~> 1.12"])
    s.add_dependency(%q<rake>, ["~> 10.0"])
  end
end
